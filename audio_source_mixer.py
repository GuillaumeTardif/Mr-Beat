...
class AudioSourceMixer(ThreadSource):
    ...
    def get_bytes(self, *args, **kwargs):
        ...
        if self.buf is None or not len(self.buf) == step_nb_samples:  # Initialisation du buffer
            self.buf = array('h', b"\x00\x00" * step_nb_samples)

        # Silence
        if not self.is_playing:  # Ne joue rient et revois le buffer vide si on est pas en trian de jouer.
        ...
        if self.on_current_step_changed is not None:  # Va ajuster l'indicateur
            # Décalage de 1 step pour synchroniser l'affichage du step courant
